...
export class RecommandService{
  ...
  search(media, id){
    return this.http.get('https://hw8-backend-310009.wl.r.appspot.com/recommand/' + media + '/' + id );
  }
}
...
export class RecommandComponent implements OnChanges {
  @Input() media: string;
  @Input() id: string;
  public items: any;
  ...
  public title: String;
  public mobile = false;
  constructor(private service: RecommandService) { }
  ...
  ngOnChanges() {
    ...
    if (this.media === 'movie'){
      this.title = 'Recommanded Movies';
    }
    else{
      this.title = 'Recommanded TV Shows';
    }
